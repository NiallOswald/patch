...
from patch.config import Score, Timer, Problem, NullTimer
from patch.utils import ur, clear_line, import_from_path
...
def start():
    ...
    proc = subprocess.Popen(["patch-timer", str(TRIAL_LENGTH * 60)])
...
def quit():
    ...
    typer.echo("… Uploading results to patch leaderboard server")

    if ur.has_error(ur.status()):
        message = (
            typer.style("✗", fg=typer.colors.RED)
            + " Not logged in to patch leaderboard server"
        )

    elif ur.has_error(ur.timetrial(score)):
        message = (
            typer.style("✗", fg=typer.colors.RED)
            + " Post to patch leaderboard server failed"
        )

    else:
        message = (
            typer.style("✓", fg=typer.colors.GREEN)
            + " Posted score to patch leaderboard server successfully"
        )
