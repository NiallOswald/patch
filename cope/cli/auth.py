...
from cope.utils import clear_line, ur
...
def status():
    ...
    if status:
        message = (
            ...
            + f" Logged in to cope leaderboard server account {User.get()["username"]}"
        )
    else:
        message = (
            typer.style("✗", fg=typer.colors.RED)
            + " Not logged in to cope leaderboard server account"
        )
...
def login(username: str, password: str):
    ...
    if status:
        message = (
            typer.style("✓", fg=typer.colors.GREEN)
            + f" Logged in to cope leaderboard server account {username}"
        )
    else:
...
def logout():
    """Log out of cope."""
    User.clear()
    typer.echo("Logged out of cope leaderboard server")
...
def register(username: str, password: str):
    ...
    # Register the user
    typer.echo("… Registering with cope leaderboard server")
    User.set(username, password)
    ur.update_credentials(username, password)
    ...
    if reg_status:
        message = (
            typer.style("✓", fg=typer.colors.GREEN)
            + f" Registered with cope leaderboard server account {username}"
        )
    else:
